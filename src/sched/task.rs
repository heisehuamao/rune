...
pub(crate) struct Task {
    ...
    task_fut: RefCell<Pin<Box<dyn Future<Output = ()> + Send>>>,
}
...
impl Task {
    ...
    pub(crate) fn borrow_fut(&self) -> RefMut<'_, Pin<Box<dyn Future<Output = ()> + Send>>> {
        self.task_fut.borrow_mut()
    }
    pub(crate) fn new<F>(id: u64, fut: F) -> Arc<Self>
